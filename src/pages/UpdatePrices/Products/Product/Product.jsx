...
const Product = ({ product, index }) => {
  ...
  const [LocalPrice, setLocalPrice] = useState(product.LocalPrice ? product.LocalPrice : 0);
  const [freezonePrice, setFreezonePrice] = useState(product.freezonePrice ? product.freezonePrice : 0);
  const [netPrice, setNetPrice] = useState(product.netPrice ? product.netPrice : 0);
  const [syrianPrice, setSyrianPrice] = useState(product.syriaPrice ? product.syriaPrice : 0);
  const getFile = (e) => {
  ...
  const UpdateProductPrices = () => {
    let model = {
      ...
      freezonePrice: Number(freezonePrice),
      netPrice: Number(netPrice),
      syriaPrice: Number(syrianPrice),
    };
    axios
