export const china = [
  {
    label: "Huawei",
    children: [
      {
        label: "Huawei",
        children: [
          {
            label: "50 KTL",
          },
          {
            label: "60 KTL",
          },
          {
            label: "65 KTL",
          },
          {
            label: "75 KTL",
          },
          {
            label: "100 KTL",
          },
          {
            label: "110 KTL",
          },
          {
            label: "125 KTL",
          },
          {
            label: "25 KTL",
          },
          {
            label: "30 KTL",
          },
          {
            label: "2 KTL",
          },
          {
            label: "3 KTL",
          },
          {
            label: "4 KTL",
          },
          {
            label: "5 KTL",
          },
          {
            label: "6 KTL",
          },
        ],
      },
    ],
  },
  {
    label: "Felicity",
    children: [
      {
        label: "Felicity",
        children: [
          {
            label: "100 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "250 AH",
          },
          {
            label: "300 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Growatt",
    children: [
      {
        label: "Growatt",
        children: [
          {
            label: "5 KW",
          },
          {
            label: "8 KW",
          },
          {
            label: "10 KW",
          },
        ],
      },
    ],
  },
  {
    label: "Sunray",
    children: [
      {
        label: "Sunray AGS",
        children: [
          {
            label: "1.5 KW",
          },
          {
            label: "3.5 KW",
          },
          {
            label: "3.6 KW",
          },
          {
            label: "5.5 KW",
          },
          {
            label: "6.2 KW",
          },
        ],
      },
    ],
  },
  {
    label: "Must power",
    children: [
      {
        label: "Must Power AGS",
        children: [
          {
            label: "1 KW",
          },
          {
            label: "3 KW",
          },
          {
            label: "5.2 KW",
          },
        ],
      },
    ],
  },
  {
    label: "Voltronic",
    children: [
      {
        label: "Voltronic AGS",
        children: [
          {
            label: "4 KW",
          },
          {
            label: "6 KW",
          },
          {
            label: "8 KW",
          },
          {
            label: "11 KW",
          },
        ],
      },
    ],
  },
  {
    label: "Enkeel",
    children: [
      {
        label: "Enkeel AGS",
        children: [
          {
            label: "3 AH",
          },
          {
            label: "4 AH",
          },
          {
            label: "5 AH",
          },
          {
            label: "6.5 AH",
          },
          {
            label: "7 AH",
          },
          {
            label: "9 AH",
          },
        ],
      },
      {
        label: "World East",
        children: [
          {
            label: "3 AH",
          },
          {
            label: "4 AH",
          },
          {
            label: "5 AH",
          },
          {
            label: "6.5 AH",
          },
          {
            label: "7 AH",
          },
          {
            label: "9 AH",
          },
        ],
      },
      {
        label: "Megan",
        children: [
          {
            label: "3 AH",
          },
          {
            label: "4 AH",
          },
          {
            label: "5 AH",
          },
          {
            label: "6.5 AH",
          },
          {
            label: "7 AH",
          },
          {
            label: "9 AH",
          },
        ],
      },
      {
        label: "Super Power",
        children: [
          {
            label: "3 AH",
          },
          {
            label: "4 AH",
          },
          {
            label: "5 AH",
          },
          {
            label: "6.5 AH",
          },
          {
            label: "7 AH",
          },
          {
            label: "9 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Dongjin",
    children: [
      {
        label: "Donjin",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "32 AH",
          },
          {
            label: "45 AH",
          },
          {
            label: "58 AH",
          },
        ],
      },
      {
        label: "Kinfon",
        children: [
          {
            label: "8 AH",
          },
          {
            label: "14 AH",
          },
          {
            label: "22 AH",
          },
          {
            label: "28 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "70 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Longi",
    children: [
      {
        label: "Himo 6",
        children: [
          {
            label: "570 W",
          },
          {
            label: "575 W",
          },
        ],
      },
      {
        label: "Himo 5",
        children: [
          {
            label: "540 W",
          },
          {
            label: "545 W",
          },
          {
            label: "550 W",
          },
          {
            label: "555 W",
          },
          {
            label: "bi facial 540 W",
          },
          {
            label: "bi facial 550 W",
          },
        ],
      },
    ],
  },
  {
    label: "Jinhua Jujiangngi",
    children: [
      {
        label: "Golden Power",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "Rambo",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Suoer",
    children: [
      {
        label: "Suoer Charger",
        children: [
          {
            label: "5 A",
          },
          {
            label: "10 A",
          },
          {
            label: "20 A",
          },
          {
            label: "30 A",
          },
          {
            label: "40 A",
          },
          {
            label: "50 A",
          },
        ],
      },
      {
        label: "Suoer Inverter",
        children: [
          {
            label: "1.5 KVA",
          },
          {
            label: "2 KVA",
          },
        ],
      },
      {
        label: "Suoer Controller PWM",
        children: [
          {
            label: "10 A",
          },
          {
            label: "20 A",
          },
          {
            label: "30 A",
          },
          {
            label: "40 A",
          },
          {
            label: "50 A",
          },
          {
            label: "60 A",
          },
        ],
      },
      {
        label: "Suoer Controller MPPT",
        children: [
          {
            label: "20 A",
          },
        ],
      },
    ],
  },
];

export const india = [
  {
    label: "Pacer",
    children: [
      {
        label: "Pacer",
        children: [
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
      {
        label: "Infinity",
        children: [
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Z Power",
    children: [
      {
        label: "3M",
        children: [
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
    ],
  },
  {
    label: "Exide",
    children: [
      {
        label: "EXIDE",
        children: [
          {
            label: "115 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
      {
        label: "INDEX",
        children: [
          {
            label: "115 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
      {
        label: "CEIL",
        children: [
          {
            label: "115 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
          {
            label: "230 AH",
          },
          {
            label: "240 AH",
          },
        ],
      },
    ],
  },
  {
    label: "LUMINOUS",
    children: [
      {
        label: "LUMINOUS",
        children: [
          {
            label: "100 AH",
          },
          {
            label: "125 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "175 AH",
          },
          {
            label: "200 AH",
          },
          {
            label: "220 AH",
          },
        ],
      },
      {
        label: "LUMINOUS INVERTERS",
        children: [
          {
            label: "900 W",
          },
          {
            label: "1100 W",
          },
          {
            label: "1400 W",
          },
          {
            label: "2800 W",
          },
        ],
      },
    ],
  },
  {
    label: "DEEPAK",
    children: [
      {
        label: "ORBIT",
        children: [
          {
            label: "55 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
];

export const south_korea = [
  {
    label: "DN Automotive",
    children: [
      {
        label: "KOREANA",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "VOLTEX",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "KINGSLIGHT",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "DTR",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "FLASH",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
  {
    label: "SEBANG",
    children: [
      {
        label: "SUPREME",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "MEGAPOWER",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "ROCKET",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "MAXTORM",
        children: [
          {
            label: "30 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "66 AH",
          },
          {
            label: "70 AH",
          },
          {
            label: "74 AH",
          },
          {
            label: "90 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
];

export const veitnam = [
  {
    label: "SEBANG VINA",
    children: [
      {
        label: "ROCKET VRLA",
        children: [
          {
            label: "4.5 AH",
          },
          {
            label: "7 AH",
          },
          {
            label: "9 AH",
          },
          {
            label: "12 AH",
          },
          {
            label: "18 AH",
          },
          {
            label: "26 AH",
          },
          {
            label: "33 AH",
          },
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "65 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
];

export const thailand = [
  {
    label: "3K",
    children: [
      {
        label: "3K",
        children: [
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
];

export const oman = [
  {
    label: "REEM Battery",
    children: [
      {
        label: "GRAND STAR",
        children: [
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
      {
        label: "DAIMOND POWER",
        children: [
          {
            label: "40 AH",
          },
          {
            label: "55 AH",
          },
          {
            label: "100 AH",
          },
          {
            label: "150 AH",
          },
          {
            label: "200 AH",
          },
        ],
      },
    ],
  },
];

export const availabiltyStatus = ["all", "available", "coming", "booked"];

export const priceOnLocation = ["local", "freezone"];

export const priceCurrency = ["USD", "AED"];

export const exporters = [
  {
    name: "AGS",
    value:
      "AGS INTERNATIONAL GENERAL TRADING LLC DEIRA NAIF,AL MAKTOUM HOSPITAL ROAD CONTACT:+971 558952656, Email: info@jalil.ae",
  },
  {
    name: "AJC",
    value:
      "ABDULJALIL CHHADA AUTO SPARE PARTS TRADING LLC DEIRA NAIF, AL MAKTOUM HOSPITAL ROAD    CONTACT:+971 558952656,   Email: info@jalil.ae",
  },
  {
    name: "lONGI",
    value:
      "LONGI SOLAR TECHNOLOGY Co.,Ltd NO. 8369 SHANGYUAN ROAD, ECO- INDUSTRIAL PARK, ECONOMIC AND TECHNOLOGICAL DEVELOPMENT ZONE, XIAN, CHINA",
  },
];

export const notify_partys = [
  {
    name: "HMC",
    value:
      "HMC INTERNATIONAL FZCO JEBEL ALI FREE ZONE- DUBAI- UNITED ARAB EMIRATES CONTACT : +971565527684, EMAIL : info@HMCFZ.CO",
  },
  {
    name: "CHARTER PARTY",
    value: "CHARTER PARTY DWC LLC DWC - Dubai - UAE Tel : 042420808 Email: info@azalshipping.com",
  },
  {
    name: "Alkawareer",
    value:
      "Alkawareer establishment for export and import TEl 00962798330333 ALEMAIL :qalam.clearance@gmail.com Address:Alzarqa Free Zone,Jordan",
  },
  {
    name: "BLUE LOGISTICS",
    value:
      "BLUE LOGISTICS SERVICES SARL PATRIARCHE MEOUCHY STREET NBR 4, KAHALE BLDG NBR 44, 2ND FLOOR, HORCH TABET,SIN EL FIL - LEBANONTEL: +9613242412 Email : riad.abdou@bluelogistics-lb.com VAT NBR# 601-3369083",
  },
];

export const party_of_discharge = [
  { name: "JABAL ALI , DUBAI", value: "JABAL ALI , DUBAI" },
  { name: "CHINA", value: "CHINA" },
  { name: "KOREA", value: "KOREA" },
];

export const final_distination = [
  { name: "JABAL ALI , DUBAI", value: "JABAL ALI , DUBAI" },
  { name: "Mersin , Turkey", value: "Mersin , Turkey" },
  { name: "AQABA , JORDAN", value: "AQABA , JORDAN" },
];

export const terms_and_conditions = [{ term: "Advance Payment 30% Balance to be paid time providing copy of BL." }];
export const terms_and_conditions_pricelist = [
  { term: "All prices valid for 3 days from quotation date ." },
  { term: "All prices valid for 7 days from quotation date ." },
  { term: "All prices valid for 10 days from quotation date ." },
];

export const terms_collections = [
  {
    collection: "EXWAREHOUSE",
    terms: [
      "Payment will be made either Cheque / Transfer",
      "PI is not including vat. Vat is applicable for with in UAE or GCC Countries",
      "Deposit is not refundable in any circumtance",
      "Currency Exchange Rate USD Against AED 3.674",
      "Goods will be delivered with in 7 days after completion of full payment",
      "We are not responsible any delay happened due to delay from manufacturer / shipment",
      "Quotation is valid for 7 days from the date of PI date and it would be changed without any prior notice",
    ],
  },
  {
    collection: "FOB",
    terms: [
      "Payment will be made either Cheque / Transfer",
      "PI is not including vat. Vat is applicable for with in UAE or GCC Countries",
      "Deposit is not refundable in any circumtance",
      "Currency Exchange Rate USD Against AED 3.674",
      "Goods will be delivered with in 7 days after completion of full payment",
      "We are not responsible any delay happened due to delay from manufacturer / shipment",
      "Quotation is valid for 7 days from the date of PI date and it would be changed without any prior notice",
    ],
  },
  {
    collection: "CIF",
    terms: [
      "Payment will be made either Cheque / Transfer",
      "PI is not including vat. Vat is applicable for with in UAE or GCC Countries",
      "Deposit is not refundable in any circumtance",
      "Currency Exchange Rate USD Against AED 3.674",
      "Goods will be delivered with in 7 days after completion of full payment",
      "We are not responsible any delay happened due to delay from manufacturer / shipment",
      "Quotation is valid for 7 days from the date of PI date and it would be changed without any prior notice",
      "Any variation in shipment and insurance charges  to be added with exising PI for balance amount",
    ],
  },
];

export const bank_details = [
  {
    collection: "AGS USD",
    terms: [
      "BENEFICIARY BANK:	BANQUE MISR",
      "BENEFICIARY NAME:	ABDULJALIL CHHADA AUTO SPARE PARTS TRADING L.L.C",
      "CURRENCY:	USD",
      "ACCOUNT NO: 	80415000000208",
      "SWIFT CODE :	BMISAEAA",
      "IBAN :	AE930150080415000000208",
      "BANK ADDRESS :	THIRD -41 27TH ST - PORT SAEED-DUBAI-U.A.E",
    ],
  },
  {
    collection: "AGS AED",
    terms: [
      "BENEFICIARY BANK:	BANQUE MISR",
      "BENEFICIARY NAME:	ABDULJALIL CHHADA AUTO SPARE PARTS TRADING L.L.C",
      "CURRENCY:	AED",
      "ACCOUNT NO: 	80402400001998",
      "SWIFT CODE :	BMISAEAA",
      "IBAN :	AE540150080402400001998",
      "BANK ADDRESS :	THIRD -41 27TH ST - PORT SAEED-DUBAI-U.A.E",
    ],
  },
  {
    collection: "Abduljalil Chhada / USD",
    terms: [
      "BENEFICIARY BANK:	BANQUE MISR",
      "BENEFICIARY NAME:	ABDULJALIL CHHADA AUTO SPARE PARTS TRADING L.L.C",
      "CURRENCY:	USD",
      "ACCOUNT NO: 	80315000005331",
      "SWIFT CODE :	BMISAEAA",
      "IBAN :	AE350150080315000005331",
      "BANK ADDRESS :	THIRD -41 27TH ST - PORT SAEED-DUBAI-U.A.E",
    ],
  },
  {
    collection: "Abduljalil Chhada / AED",
    terms: [
      "BENEFICIARY BANK:	BANQUE MISR",
      "BENEFICIARY NAME:	ABDULJALIL CHHADA AUTO SPARE PARTS TRADING L.L.C",
      "CURRENCY:	AED",
      "ACCOUNT NO: 	80302400008196",
      "SWIFT CODE :	BMISAEAA",
      "IBAN :	AE710150080302400008196",
      "BANK ADDRESS :	THIRD -41 27TH ST - PORT SAEED-DUBAI-U.A.E",
    ],
  },
  {
    collection: "AGS SIB",
    terms: [
      "BENEFICIARY BANK:	SHARJAH ISLAMIC BANK",
      "BENEFICIARY NAME:	AGS INTERNATIONAL GENERAL TR LLC",
      "CURRENCY:	AED",
      "ACCOUNT NO: 	0012159684001",
      "SWIFT CODE :	NBSHAEASXXX",
      "IBAN :	AE510410000012159684001",
    ],
  },
];
